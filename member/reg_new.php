 ...
 * @edit           赛邮云 http://www.mysubmail.com
 */
...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
		...
		//赛邮云 start
        $row = $dsql->GetOne("SELECT mid FROM `#@__member` WHERE phone LIKE '$phone' ");
            if(is_array($row))
            {
                ShowMsg('你使用的手机号已经被另一帐号注册，请使其它手机号！', '-1');
                exit();
            }
		//赛邮云 end
		...
		//赛邮云 start
		$mobile_reg_row = $dsql->GetOne("SELECT value FROM `#@__sysconfig` WHERE varname = 'cfg_mobile_reg' ");
         if(strlen(trim($_POST['mobile_code']))<6&&$mobile_reg_row['value'] == "Y")
        {
            ShowMsg('手机验证码错误', '-1');
            exit();
        }

         if(trim($_POST['mobile_code'])!= $_COOKIE['mobile_vcode']  && $mobile_reg_row['value'] == "Y")
        {
            ShowMsg('手机验证码不正确', '-1');
            exit();
        }
        ...
        `matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip`,`phone` )
       VALUES ('$mtype','$userid','$pwd','$uname','$sex','10','$dfmoney','$email','$dfscores',
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip','$phone'); ";
	   //添加 phone字段  end
